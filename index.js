...
var todoApp = function todoApp() {
  ...
  function updateContent() {
    ...
    textNode.getAttribute('contentEditable') == 'true'
      ? (textNode.setAttribute('contentEditable', false),
        updateStore(textNode.textContent, uid))
  ...
  function doneContent() {
    ...
    if (textNode.getAttribute('done') == 'true') {
      ...
      textNode.setAttribute('contentEditable', false);
      disableButton(
        this.closest('.content_container').children[2].childNodes[1],
        this
      );
  ...
  function deleteItemFromStorage(uid) {
    var storeArray = retrieveStore(storeName);
    storeArray = storeArray.filter(elem => {
      return elem.uid != uid;
    });
    setStore(storeName, JSON.stringify(storeArray));
  }
  ...
  function restoreList() {
    var storeArray = retrieveStore(storeName);
    if (storeArray)
      storeArray.map(elem => createElems(elem.content, elem.uid, elem.done));
  }
